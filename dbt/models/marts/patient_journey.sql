{{
  config(
    materialized='table',
    description='Comprehensive view of patient journey with correct prescription handling'
  )
}}
...
prescription_details AS (
  ...
  FROM {{ ref('prescriptions_enhanced') }} p
),
